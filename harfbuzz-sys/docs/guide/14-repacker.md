# The GSUB/GPOS repacker

*Adapted from the HarfBuzz 14.3.0 documentation, which is licensed under the terms in the HarfBuzz `COPYING` file.*

## Introduction

Several tables in the OpenType format are formed internally by a graph of subtables. Parent nodes
...
subtable and a child is more than 65,535 bytes then it's not possible for the offset to encode that
edge.
...
*  If a subtable has many parents this can result in the link from the furthest parent(s)
   being at risk for overflows. In these cases it's possible to duplicate the shared subtable which
   allows it to be placed closer to its parent.
...
*  In GSUB/GPOS overflows from Lookup subtables can be resolved by changing the Lookup to an extension
   lookup which uses a 32 bit offset instead of a 16 bit offset.

In general there isn't a simple solution to produce an optimal topological ordering for a given graph.
Finding an ordering which doesn't overflow is an NP hard problem. Existing solutions use heuristics
...
There are four key pieces to the HarfBuzz approach:
...
[`src/hb-repacker.hh`](https://github.com/harfbuzz/harfbuzz/blob/main/src/hb-repacker.hh).
...
plus a priority modifier (used to change where nodes are placed by increasing or
...
[Dijkstra's algorithm](https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm). Then the topological
ordering is produced by applying a modified version of Kahn's algorithm that uses a priority queue
...
to be as fast as possible. There are a few things that are done to speed up subsequent sorting
operations:
...
the topological sorting algorithm. Currently a basic heap based queue is used. Heap based queues
...
If a graph contains multiple 32 bit offsets then the shortest distance sorting will likely be
...
to pack the first subgraph in its entirety and then have the second subgraph packed after with the 32
...
this is often not the case. So this idea can be generalized as follows:
...
node and its descendants to be packed between all nodes in space n-1 and space n+1. Resulting in a
...
For certain table types we can preprocess and modify the graph structure to reduce the occurrences
...
4.  While set `S` is not empty:
    ...
    b.  During traversal if a node, `m`, has an edge to a node in space 0 then `m` must be duplicated
        to disconnect it from space 0.

    c.  Remove all nodes in `Q` from `S` and assign all nodes in `Q` to `next_space`.

    d.  Increment `next_space` by one.
...
*  If the overflowing offset is inside a space other than space 0 and the subgraph space has more
   than one 32 bit offset pointing into the subgraph then subdivide the space by moving the subgraph
...
*  If the overflowing offset is pointing to a subtable with more than one incoming edge: duplicate
   the node so that the overflowing offset is pointing at its own copy of that node.

*  Otherwise, attempt to move the child subtable closer to its parent. This is accomplished by
...
The HarfBuzz repacker has tests defined using generic graphs in
[`src/test-repacker.cc`](https://github.com/harfbuzz/harfbuzz/blob/main/src/test-repacker.cc).
...
*  Currently only children nodes are moved to resolve offsets. However, in many cases moving a parent
   node closer to its children will have less impact on the size of other offsets. Thus the algorithm
