...
are written to the struct's fields.
...
Complex tables are made up of graphs of objects, where offsets are used
...
Each packed object is associated with an ID, its zero based position in the packed
...
exact same bytes and all of its links are equivalent. If an equivalent object is found the
...
That process is documented in the repacker documentation,
[`docs/repacker.md`](https://github.com/harfbuzz/harfbuzz/blob/main/docs/repacker.md),
in the HarfBuzz repository.
