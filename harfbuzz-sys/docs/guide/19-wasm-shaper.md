...
The `cluster` field is used to map glyphs in the output glyph stream back to characters in the input Unicode sequence for hit testing, cursor positioning, etc. It must be set to a monotonically increasing value across the buffer.
...
To assist the shaping code in mapping codepoints to glyphs, the WASM shaper exports the following functions. Note that these are the low level API functions; WASM authors may prefer to use higher-level abstractions around these functions, such as the [`harfbuzz-wasm` Rust crate](https://github.com/harfbuzz/harfbuzz/tree/main/src/wasm/rust/harfbuzz-wasm) provided by HarfBuzz.
...
Returns the default horizontal and vertical advance respectively for the given glyph ID at the current scale and variations settings.
...
uint32 face_create(blob_t* blob)
```
...
```C
uint32 face_get_upem(uint32 face_token)
```
...
* We need the target to be a dynamic library, and we're going to use `bindgen` to export our Rust function to WASM, so let's put these lines in the `Cargo.toml`. The HarfBuzz sources contain a Rust crate at [`src/wasm/rust/harfbuzz-wasm`](https://github.com/harfbuzz/harfbuzz/tree/main/src/wasm/rust/harfbuzz-wasm) which makes it easy to create the shaper, so we'll specify that as a dependency as well:
...
This exports a shaping function which takes five arguments, including tokens representing the shaping plan, the font and the buffer, and returns a status value. We can pass these tokens back to HarfBuzz in order to use its native functions on the font and buffer objects. More on native functions later - let's get this shaper compiled and added into a font:
...
We provide a utility to do this called [`addTable.py`](https://github.com/harfbuzz/harfbuzz/blob/main/src/addTable.py), found in the `src/` directory of the HarfBuzz sources:
...
See the documentation for the [`harfbuzz-wasm` crate](https://github.com/harfbuzz/harfbuzz/tree/main/src/wasm/rust/harfbuzz-wasm) for all the other functions available.
